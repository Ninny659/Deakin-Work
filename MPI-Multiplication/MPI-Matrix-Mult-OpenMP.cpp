...
#pragma omp parallel default(none) shared(N, M3) private(M1, M2) 
    ...
    {
        // Dividing tasks up by the number of threads
        int chunkSize = N / numThreads;
        int startRow = numThreads * chunkSize;
        int endRow = (numThreads == numThreads - 1) ? N : startRow + chunkSize;

        // Each thread generates a portion of the matrix
...
#pragma omp parallel for schedule(guided, chunkSize)
        for (int i = 0; i < N; i++)
        {
            // OMP for reduction to sum the matrix
            #pragma omp parallel for schedule(static) reduction(+:M3[j][i])
